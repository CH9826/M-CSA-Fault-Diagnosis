...
#num_epochs = 40
def Train_CNN(DataFrame1,path_save_model,num_epochs,kernel_size,filter_number,num_steps):
    ...
    nn_model = CNN(input_shape=(X_train.shape[1],X_train.shape[2]), 
    ...
    X_tsne = tsne.fit_transform(token.reshape(X_test.shape[0], -1))  # 将X_test展平后降维  
      
    plt.figure(figsize=(8, 6))  
    scatter = plt.scatter(token[:, 0], token[:, 1], c=Y_test1)  
    plt.colorbar(scatter)  
    plt.title('t-SNE visualization of test data')  
    plt.savefig('tsne_plot_cnn.png')  
    plt.show()
    
    return loss,acc,val_loss,val_acc
    
def Test_CNN(DataFrame2,path_load_model,num_steps):  
