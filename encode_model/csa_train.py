...
from encode_model.csa_Model import *
...
def train_csa(seq_len, patch_size, num_classes, dim, depth, heads, mlp_dim, dropout, emb_dropout, epochs, lr, gamma, train_DataFrame):
    ...
    csa_model = CSA(num_classes=num_classes)
...
#     optimizer = optim.Adam(list(csa_model.parameters(), lr=lr)
    optimizer = optim.Adam(csa_model.parameters(), lr=lr)  
...
#     scheduler = StepLR(optimizer, step_size=1, gamma=gamma)
    ...
    for epoch in range(epochs):
        ...
        for data, label in tqdm(train_loader):
            ...
            output = csa_model(data)  # [16, 10, 10]
...
#                 val_output = vit_model(output)
    ...
    # 保存模型
    torch.save(csa_model.state_dict(), './saved_model/ViT/csa_model_parameters.pth')
...
#     pred_labels = []
    ...
    plot_confusion_matrix(conf_mat, ['故障1', '故障2', '故障3'], './result/csa/confusion_matrix.png')
    # TODO 绘制训练数据tsne图
    ...
    plt.savefig('./result/csa/tsne_plot_test.png')  # 保存t-SNE图  
    plt.show()  
...
def test_csa(csa_model, epochs, criterion, val_loader, device):
    # val
...
if __name__ == '__main__':
    train_DataFrame = pd.read_excel('../data_input/训练.xlsx', header=None)
    ...
    # 训练
    # loss_vit, val_loss_vit, acc_vit, val_acc_vit = train_ViT(seq_len, patch_size, num_classes, dim, depth, heads, mlp_dim, dropout, emb_dropout, epochs, lr, gamma, train_DataFrame)
    ...
    # data_path = '../data_input/正常_验证.xlsx'
    # data_path = '../data_input/外圈_验证.xlsx'
    data_path = '../data_input/内圈_验证.xlsx'
    # 1、验证 正常_验证  1
    # 2、验证 外圈_验证  2
    # 3、验证 内圈_验证  3
    ...
    vit_model.load_state_dict(torch.load('../saved_model/ViT/vit_model_parameters.pth'))
    epoch_loss, epoch_acc = test_epoch(csa_model, vit_model, epochs, nn.CrossEntropyLoss(), test_loader, device)
